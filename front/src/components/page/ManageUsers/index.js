...
class ManageUsers extends React.Component{
    constructor(props){
        super(props);
        fetchNonAdminUsers()
            .then((res) => props.usersSetData(res.users));
    }
                ...
                {this.props.users.length !== 0 ? this.props.users.map((user) =>(
                    <Card> 
                        <Grid container className={s.innerContainer} >
                            ...
                            <Grid item xs={6} className={s.container}>
                                <div className={s.userGridInner}>
                                    <p> <b>Associated Brands:</b> </p>
                                    {user.user_brands.map((brand) => <Grid container><Grid item xs={6}><p> {brand.brand.brand_name}</p></Grid> <Grid item xs={6}><Button onClick={null} variant="contained" >Remove</Button></Grid></Grid>)}
                                </div>
                            </Grid>
                            <Grid item xs={6} className={s.container}> 
                                <div className={s.userGridInner}>
